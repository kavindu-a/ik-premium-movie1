...
const BOT_NAME_FANCY = 'IK Premium Movie Bot';
...
const config = {
  ...
  RCD_IMAGE_PATH: 'https://files.soonex.biz.id/33a412f3e558.jpg',
  ...
  IMAGE_PATH: 'https://files.soonex.biz.id/b81cf42e105d.jpg',
  BOT_FOOTER: '> *IK PREMIUM MOVIE BOT*',
  BUTTON_IMAGES: { ALIVE: 'https://files.soonex.biz.id/33a412f3e558.jpg' }
...
if (!text) {
return reply(`🎬 *IK PREMIUM MOVIE BOT*
...
const axios = require("axios");
const API_KEY = "lakiya_94587a2ed33b2e3fa426587872908d5d8d84c6354f971fbfa2ef17c6d93cd3e1";

await reply("🔍 Searching Movie...");

const searchUrl = `https://nexoraapi.laksidunimsara.com/cinesubz/search?query=${encodeURIComponent(text)}&api_key=${API_KEY}`;

const search = await axios.get(searchUrl);

if (!search.data || !search.data.result || search.data.result.length < 1) {
return reply("❌ Movie Not Found");
}

const firstMovie = search.data.result[0];

const detailsUrl = `https://nexoraapi.laksidunimsara.com/cinesubz/details?url=${encodeURIComponent(firstMovie.url)}&api_key=${API_KEY}`;

const details = await axios.get(detailsUrl);

const movie = details.data.result || details.data;

if (!movie) {
return reply("❌ Details Not Found");
}

const title = movie.title || firstMovie.title || "Unknown";
const poster = movie.image || movie.poster || firstMovie.image;
const dlLink =
movie.download ||
movie.download_link ||
movie.link ||
...
let caption = `🎬 *IK PREMIUM MOVIE BOT*\n\n`;
caption += `📌 *Title:* ${title}\n`;
caption += `📅 *Year:* ${movie.year || "N/A"}\n`;
caption += `⭐ *IMDb:* ${movie.imdb || "N/A"}\n`;
caption += `🎭 *Genre:* ${movie.genre || "N/A"}\n`;
caption += `🌍 *Language:* ${movie.language || "N/A"}\n\n`;
caption += `${movie.description || "No Description"}\n`;
...
const downloadUrl =
`https://cinedl.laksidunimsara.com/movie/cinesubz?url=${encodeURIComponent(dlLink)}&api_key=${API_KEY}`;
...
reply("⚠️ Download Link Not Found");
}
